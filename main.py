...
def train_minibatch(input_data, task, label, model, **kwargs):
    ...
    adv_trainer, at_rate, at_method = kwargs["adv_trainer"], kwargs["at_rate"], kwargs["at_method"]
    if at_rate > 0:
...
def main():
    ...
    parser.add_argument("--fine_tune_task", type=Task, nargs='+', default=list([Task.tweeteval_stance]), choices=list(Task),
                        required=False, help="emobank, claire, glue_sts_b, isear_v3, tweeteval_stance, list(['tweeteval_offensive','tweeteval_hate'])")
    ...
    parser.add_argument("--at_emb_names", default="word_embedding,encoder.layer.0", type=str, help="(1) word_embedding; (2) word_embedding,encoder.layer.0")
    parser.add_argument('--at_epsilon', type=float, default=1, help='at_epsilon 1/5')
    ...
    model = InfoPLM(pretrained_model_path=args.pretrained_model_path,
    ...
    at_method, at_rate, at_epsilon, emb_names = args.at_method, args.at_rate, args.at_epsilon, args.at_emb_names
    adv_trainer = None
    if at_rate > 0:
        from at_training import FGM, PGD
        if at_method == "fgm":
            adv_trainer = FGM(model, epsilon=at_epsilon, emb_names=emb_names.split(","))
        elif at_method == "pgd":
            adv_trainer = PGD(model, epsilon=at_epsilon, alpha=0.1, emb_names=emb_names.split(","))
        else:
            raise ValueError("un-supported gradient-based adversarial training: {} !!!".format(args.at_method))
